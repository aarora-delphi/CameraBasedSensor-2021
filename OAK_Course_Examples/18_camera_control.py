...
with dai.Device(pipeline) as device:
    ...
    print(f"maxCropX - {maxCropX}, maxCropY - {maxCropY}")
    print(f"getResolutionWidth() {camRgb.getResolutionWidth()}, getResolutionHeight() {camRgb.getResolutionHeight()}")
    print(f"getVideoWidth() {camRgb.getVideoWidth()}, getVideoHeight() {camRgb.getVideoHeight()}")
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            print("Autofocus disable")
            ctrl = dai.CameraControl()
            ctrl.setAutoFocusMode(dai.CameraControl.AutoFocusMode.OFF)
            controlQueue.send(ctrl)
        elif key == ord('y'):
            print("Set autofocus region")
            ctrl = dai.CameraControl()
            ctrl.setAutoFocusRegion(int(cropX * camRgb.getResolutionWidth()),
                                    int(cropY * camRgb.getResolutionHeight()),
                                    camRgb.getVideoWidth(),
                                    camRgb.getVideoHeight()) # same region as video
            controlQueue.send(ctrl)
        elif key == ord('e'):
