...
### local-packages
from project.utilities import pickle_util
from project.utilities.logger import *

class AppControls():
    def __init__(self):
        dir_path = os.path.dirname(os.path.abspath(__file__))
        self.base_storage = f"{dir_path}/../../../storage-oak"
        self.init_camera()
        self.init_station()
        self.init_focus()
        self.init_roi()
    ...
    def set_app_active(self, is_active):
        ...
        """
        pickle_util.save(f"{self.base_storage}/drawroi_running.pb", is_active)

    def init_roi(self):
        """
            Initializes the ROI
        """
        self.pickle_roi = f"{self.base_storage}/canvas_roi.pb"
    ...
    def init_camera(self):
        ...
        """
        self.cameralist = pickle_util.load(f"{self.base_storage}/device_id.pb", error_return = ["A", "B", "C"])
        # self.cameralist = []
    ...
    def get_station(self, camera_id):
        ...
        """
        loop_num = pickle_util.load(f"{self.base_storage}/station_{camera_id}.pb", error_return = '255')
        if loop_num == '255':
    ...
    def set_station(self, camera_id, station):
        ...
        if camera_id in self.cameralist and station in self.station_dict_inv:
            ...
            pickle_util.save(f"{self.base_storage}/station_{camera_id}.pb", loop_num)
            return True
        else:
    ...
    def get_focus(self, camera_id):
        ...
        """
        focus_num = pickle_util.load(f"{self.base_storage}/focus_{camera_id}.pb", error_return = None)
    ...
    def set_focus(self, camera_id, focus):
        ...
        if camera_id in self.cameralist and focus in self.focus_dict:
            ...
            pickle_util.save(f"{self.base_storage}/focus_{camera_id}.pb", focus_num)
            return True
        else:
            if camera_id not in self.cameralist:
                log.error(f"Camera {camera_id} not in list of cameras {self.cameralist}")
    ...
    def get_view(self, camera_id):
        ...
        """
        image_path = f"{self.base_storage}/{camera_id}.png"
        blank_image_path = f"{self.base_storage}/blank.png"
